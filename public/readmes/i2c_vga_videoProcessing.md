...
`System Verilog`, `Python`, `GitHub`, `Jira` 
...
마지막으로, 한 명이 만든 모듈이 전체 시스템 흐름에 큰 영향을 미치기 때문에, 팀원 간 명확한 인터페이스 정의와 사전 시뮬레이션, 디버깅 기준 공유가 중요함을 실감했다. 협업 과정에서 시스템 레벨의 조율과 통합 테스트의 중요성을 배우는 귀중한 기회가 되었고, 이 경험은 향후 더 복잡한 SoC 설계나 실제 반도체 제품 개발 시에도 중요한 기반이 될 것이라 생각한다.
</details>